  ...
  methods: {
    initData () {
      if (this.paramData) {
        ...
        const { address, box, createTime, idCard, name, phone, postNum, uuid, weight, length, width, height, bulky } = this.paramData
        this.$nextTick(() => {
          setFieldsValue({
            address, box, createTime, idCard, name, phone, postNum, uuid, weight, length, width, height, bulky
          })
        })
      }
      // 存在uuid 需要查一下商品
